...
instance (Applicative m, Show1 m) => Show1 (SourceT m) where
  liftShowsPrec sp sl d (SourceT m) =
    showsUnaryWith
      (liftShowsPrec sp sl)
      "fromStepT"
      d
...
fromActionStep stop action = loop
  where
...
transformStepWithAtto
  :: forall a m
   . Monad m
  => A.Parser a
  -> StepT m BS.ByteString
  -> StepT m a
transformStepWithAtto parser = go (A.parse parser)
  where
    p0 = A.parse parser

    go
      :: (BS.ByteString -> A.Result a)
      -> StepT m BS.ByteString
      -> StepT m a
